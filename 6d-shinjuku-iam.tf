...
resource "aws_secretsmanager_secret" "shinjuku_db_creds_v9" {
  provider    = aws.shinjuku
  name        = "lab-3a/shinjuku/rds/mysql_v9"
  description = "Authoritative DB credentials + endpoint for Lab 3A (Shinjuku only)"

  tags = merge(local.tags, {
    Name = "lab-3a-shinjuku-rds-creds-v9"
    Role = "authoritative"
  })
}

# Create a new version of the Secrets Manager secret with Shinjuku RDS credentials
resource "aws_secretsmanager_secret_version" "shinjuku_db_creds_v9" {
  provider  = aws.shinjuku
  secret_id = aws_secretsmanager_secret.shinjuku_db_creds_v9.id
...
resource "aws_ssm_parameter" "shinjuku_rds_endpoint" {
  provider    = aws.shinjuku
  name        = "/lab-3a/shinjuku/rds_endpoint"
  description = "RDS endpoint for Lab 3A (Shinjuku only)"
  type        = "SecureString"
  value       = aws_db_instance.shinjuku.address

  tags = merge(local.tags, {
    Name = "lab-3a-shinjuku-rds-endpoint"
  })
}