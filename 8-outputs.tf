...
First note:     https://${local.zone_name}/add?note=test_from_bonus_f
...
Fifth note:     https://${local.zone_name}/add?note=lab_1c_bonus_f_is_successful
...
2. Open in browser:
   ...
   http://localhost:80/add?note=lab_1c_bonus_f_is_successful
   http://localhost:80/list
EOT
}

output "bonus_f_logs_insights_waf_log_group" {
  value       = local.waf_cw_log_group_name
  description = "WAF CloudWatch log group used by Logs Insights queries."
}

output "bonus_f_logs_insights_app_log_group" {
