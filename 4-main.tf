...
resource "aws_wafv2_web_acl_logging_configuration" "bonus_g_waf_logging" {
  count = (var.enable_waf && var.waf_log_destination == "cloudwatch") ? 1 : 0

  resource_arn = aws_wafv2_web_acl.waf[0].arn
  log_destination_configs = [
    aws_cloudwatch_log_group.bonus_g_waf_log_group[0].arn
  ]
...
resource "aws_cloudwatch_log_group" "bonus_g_waf_log_group" {
  count             = (var.enable_waf && var.waf_log_destination == "cloudwatch") ? 1 : 0
