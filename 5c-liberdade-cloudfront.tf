...
resource "aws_cloudfront_distribution" "liberdade_cf" {
  ...
  default_cache_behavior {
    ...
    cache_policy_id          = aws_cloudfront_cache_policy.liberdade_no_cache.id
...
# CloudFront policies (no-cache + forward query strings)
resource "aws_cloudfront_cache_policy" "liberdade_no_cache" {
  provider    = aws.liberdade
  name        = "${local.liberdade_prefix}-no-cache"
  comment     = "Disable caching for dynamic notes app"
  default_ttl = 0
  max_ttl     = 0
  min_ttl     = 0
