...
#
# Located in: scripts/
# Outputs to: ./scripts-results/gate_result_secrets_and_role.json
# ============================================================
...
OUT_JSON="${OUT_JSON:-./scripts-results/gate_result_secrets_and_role.json}"
...
EXPECTED_ROLE_NAME="${EXPECTED_ROLE_NAME:-ec2-to-secrets-role}"
...
usage() {
  cat <<EOF
Usage:
  REGION=sa-east-1 INSTANCE_ID=i-05bf561b96e642ca0 SECRET_ID=arn:aws:secretsmanager:sa-east-1:866340886126:secret:lab-rds-mysql-5i3FJD scripts/gate_secrets_and_role.sh
...
Optional toggles (env vars):
  ...
  OUT_JSON=...                               (default: ./scripts-results/gate_result_secrets_and_role.json)
EOF
}
