...
echo "STATIC_EXAMPLE=$(date -u +%Y%m%dT%H%M%SZ)" > /var/www/html/static/example.txt

# Dynamic API endpoint via CGI at /api/list
cat > /etc/httpd/conf.d/lab2b-api.conf <<'EOF'
...
chmod 755 /var/www/cgi-bin/list
